...
class ManimRendererTool:
    """Tool to generate videos using Manim renderer service."""
    
    def __init__(self, manim_renderer_url: str):
        """Initialize the Manim renderer tool.
        
        Args:
            manim_renderer_url: Base URL of the Manim renderer service
        """
        self.manim_renderer_url = manim_renderer_url.rstrip('/')
        self.render_endpoint = f"{self.manim_renderer_url}/render"
        self.status_endpoint = f"{self.manim_renderer_url}/status"
    ...
    def generate_video(
        ...
        """Generate a video using Manim code.
        
        Args:
            code: Python code with Manim animations
            user_id: User identifier
            chat_id: Chat identifier
            classroom_id: Classroom identifier
            subject_id: Subject identifier
            scene_name: Optional scene class name
        ...
        Returns:
            Dict containing success status, video URL, and details
        """
        try:
            ...
            if not job_id:
                return {
                    "success": False,
                    "error": "No job ID returned from renderer",
            ...
            for _ in range(max_polls):
                ...
                if status_data.get("status") == "completed":
                    ...
                    # Note: The actual video URL would be from job info
                    return {
                        "success": True,
                    ...
                    return {
                        "success": False,
                        "error": f"Video generation failed: {status_data.get('status')}",
                        "job_id": job_id,
                        "details": status_data
                    }
            ...
            return {
                "success": False,
                "error": f"Renderer service request failed: {str(e)}",
                "message": "Failed to communicate with video renderer"
            }
        except Exception as e:
            return {
                "success": False,
